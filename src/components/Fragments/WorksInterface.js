  ...
  return (
    <div>
      <div className="mx-auto my-7 max-w-5xl rounded-[2rem] border border-slate-700 py-12 dark:border-slate-400 lg:max-w-xl md:max-w-lg sm:my-6 sm:max-w-xs sm:rounded-3xl sm:py-8">
        <TitleWorks classname="bg-blue-400/10 text-blue-400 dark:bg-blue-400/20" title="User Interface Design"></TitleWorks>
            ...
            {WorksInterfaceData.map((workInterface) => {
              return (
                <div key={workInterface.id}>
                  <Image link={workInterface.link} image={workInterface.image}></Image>
                  <div className="max-w-md sm:max-w-[16rem]">
                    <div className="flex justify-between sm:block">
                      <Name classname={workInterface.css} name={workInterface.name}></Name>
                      <Link link={workInterface.link} live={workInterface.live} platform="Figma"></Link>
                    </div>
                  </div>
                </div>
              );
